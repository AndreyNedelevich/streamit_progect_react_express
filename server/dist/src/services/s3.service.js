...
class S3Service {
    constructor(client = new client_s3_1.S3Client({
        region: config_1.configs.AWS_REGION,
        credentials: {
            accessKeyId: config_1.configs.AWS_ACCESS,
            secretAccessKey: config_1.configs.AWS_KEY,
        },
    })) {
        this.client = client;
    }
    async uploadFile(file, itemType, itemId) {
        ...
        await this.client.send(new client_s3_1.PutObjectCommand({
            Bucket: config_1.configs.AWS_NAME,
            Key: filePath,
            Body: file.data,
    ...
    async uploadFileStream(stream, itemType, itemId, file) {
        const filePath = this.buildPath(itemType, itemId, file.name);
        await this.client.send(new client_s3_1.PutObjectCommand({
            Bucket: config_1.configs.AWS_NAME,
            Key: filePath,
